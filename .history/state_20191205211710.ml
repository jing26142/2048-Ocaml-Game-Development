...
let rec str_list row =
  ...
  |h::t -> string_of_int(h) ^ "," ^ (str_list t)
             ...
             lelet grid_lst =  o_t (List.nth grid_lst 0) in
let row1 = str to_matrix st.current_grid in
let row0 = str_list (List.nth grid_lst 2) in
