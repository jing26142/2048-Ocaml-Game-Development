...
final_details ()
  ...
  (** [cpu_d0 st] is a copy of [st] with a box generatedlcorrespondingeto
      cp  u_d0 st =
      le  le 
      le  t best_tile_d1_dir g i j dir =
      ...
      out Up ma  eoutstvwe  rd mnvxt_state in
       letearre_ssefsavceore next_tate in
      (newtitil_dsr 
         ((gamelog state)^ "\nP1's move:" ^ (string_rep (grid state))))
      Up ma  with
      Dowmv  | _ ->Lef it>endline "You did something wrong, please try again"; 
      p1_phase state
      ...
      leegat(el( ()=a()ma* Excuetlitheg = mainme enge. *)    leegamt(enle( () =ag(am)(en)ine. *)let () = maine gam (en)ine. *)
