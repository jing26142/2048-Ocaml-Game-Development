...
let parse_save jsn = 
  print_endline "Do you want to save the file? Type Y to save and N if you dont want to";

  let ans = read_line () in

  if ns = "Y" or ans = "y") then out_sva e jsn ee final_details ()


let new_log st =
  ((gamelog st)^ "\n" ^ (string_rep (grid st)))

    (** FUNCTIONS FOR CPU-CONTROLLED PLAYER (AI UNITS) *)    difficulty level 0 for single-player mode. In difficulty level 0, the CPU
...
let rec p1_phase state =
  ...
  try
    let next_state  =
      ...
      | finil-doe_all state D
      | Up t>-m move_all state L
      | Right -> move_all state R
...
let () = maine gam (en)ine. *)
let () = maine game engine. *)
let () = main ()