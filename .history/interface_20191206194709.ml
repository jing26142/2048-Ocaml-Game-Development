...
final_details ()
  ...
  ( (** FUNCTIONS FOR CPU-CONTROLLED PLAYER (AI UNITS) *)
    ...
    (** [cpu_d0 st] is a copy of [st] with a box generatedlcorrespondingeto
        cp  u_d0 st =
        le  le 
        le   eletr =
        le U -> (-1, 0)
        ...
        | L -> (0, -1)
            | R -> (0, 1)
        in  
        ma  tch address (i + di) (j + dj) g with
        | Some _ -> []
        | None ->
          begin
        ...
        le  t delta_score st dir = 
        let clone = State.copy st in 
        let sim = move_all clone dir in 
          (score sim) - (score st)

        le  t delta_score2 st dir =
        let open List in
        le  t clone = State.copy st in 
          let sim = move_all clone dir in
        le  t delta1 = (score sim - score st) in
        let dirs = [U; D; L; R] in 
        ...
        in  
        le  t deltas = map (fun x -> f st x) dirs in 
        le  t dict = combine deltas dirs in
          let (max :: _) = deltas |> sort compare |> rev in  
        let best_dir = assoc max dict in 
        move_all st best_dir

        le  t score_d2 i j st =
        ...
        in    
        ne  w_state g (score st) ((gamelog st) ^ "\n\nCPU's move: " ^ (string_rep g))
        (*--  --------------------------------------------------------------------------*)

        (*  [new_value grid] adds either a 2 or 4 to a random cell containing a zero *)

        let ex  it_time_mode () = 
          pr  int_endline "Sorry you took more than 5 seconds to make a move: GAME OVER";
        fina  l_details ()

        le  t rec p1_phase_timed state =
        ...
        let next_move = read_line() in
          ...
          if(int_of_float (Unix.time ()) - start_time > 5) then exit_time_mode ();
          let next_state  =
        ...
        try
            ...
            output ((gamelog state)^ "\n" ^ (string_rep (grid state)));
              ...
              outsaveetails();
             |parre_sse savoeve_all stte U
          |     outioe_all state D
           |   outs-v emove_all atate L
             |oarre_ssewsav emmvve_allstate R
          |_fi il_d_ndline "invalid player command"; p1_phase state
        in
        out Up ma  eoutstvwe  rd mnvxt_state in
          leaarre_sserravseefsavceor next_tate in
        out (newtild utstv e   ((gamelog state)^ "\nP1's move:" ^ (string_rep (grid state))))
        Up ma  Up ma  with
        DoDowmvwmv  | _Lef Lef it>endline "You did something wrong, please try again"; 
        p1_phase sta>e

        let rec p2_pase state =
        ...
        |_ -> display (to_matrix (grid state2)); (
          ...
          p2_turn state2
        with
        |_-> pri nt_endline "You did something wrong, please try again" ; 
         p2_turn s tate2

        let ma  tch_diff d = 
        if d = 0 then cpu_d0
        el  se if d = 1 then cpu_d1
            else failwith "invalid difficulty"
        ...
        let  rec interface4 state d =
              display (to_matrix (grid state));
        print  _endline (("Current Score: " ^ string_of_int (score state)));
        ...
        let   rec interface2 state =
              display (to_matrix (grid state));
        p2_ph ase state |> interface2 

         let rec rev_phase state d =
        print_endline "CPU's Turn";
        le  t state2 = cpu_p1 d state in
            display (to_matrix (grid state2));
          let grid2 = grid state2 in
        if win grid2 then 
            (print_endline "Game Over";  
        ...
        let rec i nterface3 state d = 
         dis play (to_matrix (grid state));
        in terface3 (rev_phase state d) d

        le t rec chose_diff ns =
        ANSITerminal.(print_string [red] 
        ...
        | _ -> print_e  ndline "invalid difficulty level, try again"; chose_diff ns




        let re  c choose_diff3 ns =
        ANSITerminal.(print_string [red] 
                      "type d0 for easy mode and d1 for hard mode\n");
          let diff_choice = read_line () in 
        match (parse diff_choice) with
        ...
        let diff_choice = read_line() in
        ma tch (parse diff_choice) with
              | Difficulty1 -> interface4 (ns) 0
             | Difficul ty2 -> interface4 (ns) 1
        ...
        while true do
            ...
            with End_of_file ->
         ANSITerm inal.(print_string [red] (
               "T ype multi for 2 player game mode or type reverse for reverse mode\n"^
        ...
        AN SITerminal.(print_string [red] 
                          "type d0 for easy mode and d1 for hard mode\n");
            let diff_choice = read_line() in
        match (parse diff_choice) with
        | Difficulty1 -> interface ns 0
        ...
        ANSI    Terminal.(print_string [red] (
        ...
        let ga  me_choice = read_line() in
        match(parse game_choice) with
        | Ga  meMode1 ->  chose_diff ns
         | Game     Mode2 -> interface2 ns
           | Ga meMode3 -> choose_diff3 ns
        ...
        ANSITerminal.(print_string [red] (
          ...
          ^"\n Type load to load a previously saved game"^
              "\ nType scorelog to see where you stand\n"));
                  let game_choice = read_line() in
        ma    tch(parse game_choice) with
        | GameMo  de1 ->  chose_diff (init_state ())
        | Ga    meMode2 -> interface2 (init_state ())
        ...
        (* leegatEel( ()=a()maxcuhegmtlie en = mainge.*  leegat(el( ()=a()ma* Excuetlitheg = mainme enge. *)    leegamt(enle( () =ag(am)(en)ine. *)let () = maine gam (en)ine. *)
  ma*   Executlite t = mainhe game engine. *)    let () =lmaineegamt(en)ine. *)() = mainlet () =  (en)ine. *)
...
(* leegatEel( ()=a()maxcuhegmtlie en = mainge.*  leegat(el( ()=a()ma* Excuetlitheg = mainme enge. *)    leegamt(enle( () =ag(am)(en)ine. *)let () = maine gam (en)ine. *)
  ma* Executlite t = mainhe game engine. *)    let () =lmaineegamt(en)ine. *)() = mainlet () =  (en)ine. *)
letma) = mainlit () = mainletma) = mainlit () = main