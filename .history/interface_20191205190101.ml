...
let output log =
  let txtfile = open_out "./gamelog.rtf" in
      ...
      then (display (to_matrix (grid state2)); (
        ...
        if lose fgrid then
          (print_endline "Congratulation Player 2. You Win!";
           output ((gamelog state2)^ "\n" ^ (string_rep (grid state2))); final_details();)
        else 
...
let read_file () = 
  ...
  with End_of_file ->
    ...
    ANSI.terminal(print_string [red] ("type single for 1 player or type multi for 2 player game mode"));
