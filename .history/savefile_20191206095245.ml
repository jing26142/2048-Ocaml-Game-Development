...
(* let rec int_row row n =
   ...
   let row0 = int_row (List.nth grid_lst 0) in
   let row1 = int_row (List.nth grid_lst 1) in
   let row2 = int_row (List.nth grid_lst 2) in
   let row3 = int_row (List.nth grid_lst 3) in
...
let string_save st =
  let grid_lst = to_matrix st.current_grid in
  ...
  `Assoc [ ("row0", `String row0); ("row1"), `String row1;
           ...
           ("score", `Int st.score); ("gamelog", `String st.gamelog)]
