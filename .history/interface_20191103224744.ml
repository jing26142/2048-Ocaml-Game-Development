...
let rec move_left box grid =
  ...
  let r = fst bpos in
  let c = snd bpos in
  if (c = 0) then (print_endline "true"; grid)
  else (print_endline "!!"; match (address r (c-1) grid) with
    |None -> (print_endline (string_of_int r)); gen_box (vbox) r (c-1) grid |>
                                                remove_box r c |> 
...
let move_all_down grid =
  grid |> down_box (address 3 0 grid) |> right_box (address 3 1 grid)|>right_box (address 3 2 grid)
  |>right_box (address 3 3 grid) |> down_box (address 2 0 grid) |> right_box (address 2 1 grid)|>right_box (address 2 2 grid)
  |>right_box (address 2 3 grid)|> down_box (address 1 0 grid) |> right_box (address 1 1 grid)|>right_box (address 1 2 grid)
  |>right_box (address 1 3 grid)|> down_box (address 0 0 grid) |> right_box (address 0 1 grid)|>right_box (address 0 2 grid)
  |>right_box (address 0 3 grid)
