...
let rec move_down box grid =
  ...
  if ( r = (grid_size grid) -1) then grid
    ...
    |Some box2 -> if ((value box2) = value box) then
        let new_v = 2*value box in remove_box r c grid 
                                   |> remove_box (r+1) c
                                   |> gen_box new_v (r+1) c else grid



let rec down_box cell grid = 
  match cell with 
  |None -> grid 
  |Some box -> move_down box grid 


let move_all_down grid =
  grid |> down_box (address 3 0 grid) |> down_box (address 2 0 grid)
  |>down_box(address 1 0 grid) |>down_box(address 0 0 grid) 
  |> down_box (address 3 1 grid) |> down_box (address 2 1 grid)
  |>down_box (address 1 1 grid) |>down_box(address 0 1 grid) 
  |> down_box (address 3 2 grid) |> down_box (address 2 2 grid) 
  |>down_box (address 1 2 grid) |>down_box (address 0 2 grid)
  |> down_box (address 3 3 grid) |> down_box (address 2 3 grid)
  |>down_box (address 1 3 grid) |>down_box (address 0 3 grid)

let rec interface state =
  display (to_matrix (grid state));
  print_endline (string_of_int (score state));
  ...
  try
    ...
    |Down -> interface (new_state 
                          ((move_all_down (grid state))|> random)   0)
